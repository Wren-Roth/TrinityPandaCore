...
#ifndef SF_SPELL_SCRIPT_H
#define SF_SPELL_SCRIPT_H
...
class SpellScript : public _SpellScript
{
    ...
    public:
        ...
        //
        // Called before sending SMSG_SPELL_START, won't be called for triggered spells
        ...
        // Same as PreventHitEffect but can be used during launch phase as well.
        // Should only be used when hit phase is absolutely unreachable (i.e. SPELL_EFFECT_TRIGGER_SPELL)
        void PreventEffect(SpellEffIndex effIndex);

        // Same as PreventHitDefaultEffect but can be used during launch phase as well.
        // Should only be used when hit phase is absolutely unreachable (i.e. SPELL_EFFECT_TRIGGER_SPELL)
        void PreventDefaultEffect(SpellEffIndex effIndex);

        // Remove effect from mask for specified target.
        // Must be called in OnEffectLaunchTarget.
        void PreventEffectForTarget(Unit* target, SpellEffIndex effIndex);

        // HACK, currently usable only for removing main target in chain targets hook
        void RemoveUnitTarget(uint64 guid);
        ...
        Spell::UsedSpellMods const& GetAppliedSpellMods() const;

        void AddTriggerFlag(TriggerCastFlags flags);

        SpellMissInfo GetHitResult() const;
};
...
enum AuraScriptHookType
{
    ...
    AURA_SCRIPT_HOOK_EFFECT_CALC_TICK_AMOUNT,
    AURA_SCRIPT_HOOK_EFFECT_CALC_SPELLMOD,
    AURA_SCRIPT_HOOK_EFFECT_DROP_MOD_CHARGE,
    AURA_SCRIPT_HOOK_EFFECT_ABSORB,
    AURA_SCRIPT_HOOK_EFFECT_ABSORB_HEAL,
...
#define AURASCRIPT_FUNCTION_TYPE_DEFINES(CLASSNAME) \
        ...
        typedef void(CLASSNAME::*AuraEffectCalcTickAmountFnType)(AuraEffect const*, int32&); \
        typedef void(CLASSNAME::*AuraEffectCalcSpellModFnType)(AuraEffect const*, SpellModifier* &); \
        typedef void(CLASSNAME::*AuraEffectAbsorbFnType)(AuraEffect*, DamageInfo &, uint32 &); \
        typedef void(CLASSNAME::*AuraEffectAbsorbHealFnType)(AuraEffect*, uint32&, uint32&); \
        ...
        typedef void(CLASSNAME::*AuraDropModChargeFnType)(Spell*); \
        ...
        class EffectCalcTickAmountHandler : public EffectBase
        {
        public:
            EffectCalcTickAmountHandler(AuraEffectCalcTickAmountFnType handler, uint32 effIndex, uint16 effName);
            void Call(AuraScript* auraScript, AuraEffect const* eff, int32& amount);
        private:
            AuraEffectCalcTickAmountFnType _handler;
        };
        ...
        class EffectAbsorbHealHandler : public EffectBase
        {
        public:
            EffectAbsorbHealHandler(AuraEffectAbsorbHealFnType effectHandlerScript, uint8 effIndex);
            void Call(AuraScript* auraScript, AuraEffect* aurEff, uint32& heal, uint32& absorb);
        private:
            AuraEffectAbsorbHealFnType _effectHandlerScript;
        };
        ...
        class AuraDropModChargeHandler
        {
        public:
            AuraDropModChargeHandler(AuraDropModChargeFnType _pEffectHandlerScript);
            void Call(AuraScript* auraScript, Spell* bySpell);
        private:
            AuraDropModChargeFnType _HandlerScript;
        };
        ...
        class EffectCalcTickAmountHandlerFunciton : public AuraScript::EffectCalcTickAmountHandler { public: EffectCalcTickAmountHandlerFunciton(AuraEffectCalcTickAmountFnType handler, uint8 effIndex, uint16 effName) : AuraScript::EffectCalcTickAmountHandler((AuraScript::AuraEffectCalcTickAmountFnType)handler, effIndex, effName) { }  }; \
        ...
        class EffectAbsorbHealFunction : public AuraScript::EffectAbsorbHealHandler { public: EffectAbsorbHealFunction(AuraEffectAbsorbHealFnType effectHandlerScript, uint8 effIndex) : AuraScript::EffectAbsorbHealHandler((AuraScript::AuraEffectAbsorbHealFnType)effectHandlerScript, effIndex) { } }; \
        ...
        class AuraDropModChargeFunction : public AuraScript::AuraDropModChargeHandler { public: AuraDropModChargeFunction(AuraDropModChargeFnType _HandlerScript) : AuraScript::AuraDropModChargeHandler((AuraScript::AuraDropModChargeFnType)_HandlerScript) {} }; \
    ...
    public:
        ...
        // executed after calculation of the aura effect amount
        HookList<EffectCalcAmountHandler> AfterEffectCalcAmount;
        ...
        // executed when aura effect handles periodic tick
        // example: DoEffectCalcTickAmount += AuraEffectCalcTickAmountFn(class::function, EffectIndexSpecifier, EffectAuraNameSpecifier);
        // where function is: void function (AuraEffect const* aurEff, int32& amount);
        HookList<EffectCalcTickAmountHandler> DoEffectCalcTickAmount;
        #define AuraEffectCalcTickAmountFn(F, I, N)  EffectCalcTickAmountHandlerFunciton(&F, I, N)
        ...
        // executed when aura effect is going to absorb heal
        // example: OnEffectAbsorbHeal += AuraEffectAbosrbHealFn(class::function, EffectIndexSpecifier);
        // where function is: void function(AuraEffect* aurEff, uint32& heal, uint32& absorb);
        ...
        // executed when aura spellmod charge is dropped
        // example: OnDropModCharge += AuraDropModChargeFn(class::function);
        // where function is: void function(Spell* bySpell);
        HookList<AuraDropModChargeHandler> OnDropModCharge;
        #define AuraDropModChargeFn(F) AuraDropModChargeFunction(&F)
        ...
        void Remove(uint32 removeMode = AURA_REMOVE_BY_DEFAULT);
        ...
        // Sets stack amount of the aura. Can be used only during Load method call.
        void InitStackAmount(uint32 stacks);
};
