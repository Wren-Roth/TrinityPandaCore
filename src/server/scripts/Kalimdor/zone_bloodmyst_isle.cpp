...
uint32 const possibleSpawns[30] = { 17322, 17661, 17496, 17522, 17340, 17352, 17333, 17524, 17654, 17348, 17339, 17345, 17359, 17353, 17336, 17550, 17330, 17701, 17321, 17325, 17320, 17342, 17715, 17334, 17341, 17338, 17337, 17346, 17344, 17327 };
...
/*######
## Quest 9759: Sironas completion on kill
######*/

enum SironasData
{
    QUEST_SIRONAS_FINISH = 9759,
    NPC_SIRONAS = 17678
};
...
public:
    ...
    struct npc_sironasAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* killer) override
        {
            ...
            if (Player* player = killer->ToPlayer())
            {
                // If player has the quest and it's incomplete, complete it for them.
                if (player->GetQuestStatus(QUEST_SIRONAS_FINISH) == QUEST_STATUS_INCOMPLETE)
                    player->CompleteQuest(QUEST_SIRONAS_FINISH, true);
            }
            else if (Unit* owner = killer->GetOwner()) // handle pet/guardian kills
            {
                if (Player* ownerPlayer = owner->ToPlayer())
                {
                    if (ownerPlayer->GetQuestStatus(QUEST_SIRONAS_FINISH) == QUEST_STATUS_INCOMPLETE)
                        ownerPlayer->CompleteQuest(QUEST_SIRONAS_FINISH, true);
                }
            }
        }
    };
